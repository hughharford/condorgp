...
## with cgp-nt-again OR any image
...
+ access db from the local (not cluster) command line, once port forwarded
...
k port-forward service/cgp-database 5432:5432  -n cgp-system --request-timeout='0'

## check port forwarding in namespace

k get svc -n cgp-system -o json | jq '.items[] | {name:.metadata.name, p:.spec.ports[] } | select( .p.nodePort != null ) | "\(.name): localhost:\(.p.nodePort) -> \(.p.port) -> \(.p.targetPort)"'
...
k delete -f k8s/04-postgres.yaml;
k apply -f k8s/04-postgres.yaml;
...
+ in general, from outside cluster: localhost:5432
+ e.g. via DBeaver: jdbc:postgresql://localhost:5432/
+ in the cluster. e.g from Grafana: cgp-database:5432 with postgres / cgpbackbone
